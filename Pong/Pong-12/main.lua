...
function love.load()
	...
	-- set title of out application window
	...
	-- set up out sound effects; later, we can just index this table and
	...
	-- either going to be1 oor 2; whomever is scored on gets to serve the
	-- following turn
	servingPlayer = 1
...
function love.update(dt)
	...
	elseif gameState == 'play' then
		...
		-- slightly increasing it, then altering the dy based n the position of collision
		if ball:collides(player1) then
			ball.dx = -ball.dx * 1.03
			ball.x = player1.x + 5
			
			-- keep velocity going in the same direction, but rendomize it
			if ball.dy < 0 then
...
function love.draw()
	...
	-- new function just to demosntrate how to see FPS in LÖVE2D
	displayFPS()
