...
-- some fo the colors in our palette (to be used with paarticle systems)
paletteColors = {
	-- blue
	[1] = {
		['r'] = 99,
...
function Brick:init(x, y)
	...
	-- used to determin whether this brick should be rendered
	self.inPlay = true
	...
	-- lasts beteween 0.5-1 seconds 
	self.psystem:setParticleLifetime(0.5, 1)

	-- give it an acceleration of anywhere between Z1, Y1 and X2, Y2 (0, 0) and (80, 80) here
	-- gives generally downward´
	self.psystem:setLinearAcceleration(-15, 0, 15, 80)

	-- spread of particles; normal looks more natural than uniform, which is cumply; numbers
	-- are amount of standart deviation away in X and Y axis
...
function Brick:hit()
	...
	-- if we're alredy at the lowest color, else, just go down a color
	if self.tier > 0 then
