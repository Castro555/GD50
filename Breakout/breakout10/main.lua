...
function love.load()
	...
	-- seed the RNG so thar calls to random are random
    ...
    -- load up the gra+hics we'll be using troughout our states
    gTextures = {
...
function love.keypressed(key)
    -- add to our table of keys presses thhis frame
...
function love.draw()
    -- begiun drawin with push in our virtual resolution
    push:apply('start')

    -- background should be drawn regardless of state, scaled to fit our
    -- virtual resolution
    local backgroundWidth = gTextures['background']:getWidth()
    local backgroundtHeight = gTextures['background']:getHeight()
    ...
    love.graphics.draw(gTextures['background'],
        ...
        VIRTUAL_WIDTH / (backgroundWidth - 1), VIRTUAL_HEIGHT / (backgroundtHeight - 1))

    -- use the state machine to defer rendering to the current state we're in
    gStateMachine:render()
    
    -- use FPS for debugging; simply comment out to remove
    displayFPS()
...
function loadHighScores()
    ...
    -- flag for wheter we're reading a name or not
    local name = true
    local currentName = nil
    local counter = 1

    -- initializa scores table with at least 10 blank entries
    local scores = {}

    for i = 1, 10 do
        -- blank table; each will hold a name and score
        scores[i] = {
            name = nil,
            score = nil
        }
